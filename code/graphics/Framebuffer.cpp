...
bool FrameBuffer::Create()
{
	...
	if ( mFlags & eCreateColorTexture )
	{
		...
		for (int i=0; i<mNumberOfColorAttachments; ++i)
		{
			...
			{
				id = CreateTexture2D( mWidth, mHeight, mColorAttachments[i].format, mColorAttachments[i].internalFormat, mColorAttachments[i].type, &mColorAttachments[i].id, mColorAttachments[i].clamp, mColorAttachments[i].filter);
				mColorAttachments[i].deleteOnCleanup = false;
...
unsigned FrameBuffer::CreateTexture2D ( const int width, const int height, GLenum format, 
	GLenum internalFormat, GLenum type, GLuint *useId, GLenum clamp, int filter, bool generateMipMaps )
{
