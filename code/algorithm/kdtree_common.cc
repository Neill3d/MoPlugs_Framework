...
Node *build_kdtree_core(double *reference, int N, int D, int *index, 
  ...
  if ( (nodeptr = new Node) == NULL )
	{
    printf("Error allocating memory for kD-Tree\n");
		return NULL;
	}

  if ( (nodeptr->pt = new double[D]) == NULL )
	{
    printf("Error allocating memory for kD-Tree\n");
		return NULL;
	}
...
Node * rangeQuery(Node *v, double distance, double *pt, int D){
  ...
  if ( ( (v->pt[v->orientation] - distance) <=  pt[v->orientation]) &&
	...
	double dist = calcdistance(pt,v->pt,D);
    if ( dist <= distance ){
      current= new Node;
      current->pt= new double[D];
      current->right=NULL;
  ...
  for (i=0; i < M; i++) {
	...
	cur=cur->right; count++;
      ...
      *L=count;
      ...
      (*pts_in_range) = new double[count*D];
